...
> **Don't run the `.pkg` and `brew install outerloop` on the same machine.** Both start
> a hub/worker; two of them with different `INBOX_HOME`s would double-claim against the
> same GitHub repos in real mode. This `.pkg` path is for your always-on Mac; use `brew`
> only on separate boxes (see the main [README](../README.md#install)).
...
```
 phone / remote worker ──HTTPS──▶  AWS box (Caddy)  ──┬─ /api/*  → app Bearer token
   (cellular, anywhere)                               └─ /*      → browser login (basic-auth)
                                                              │
                                            AWS localhost:8765 ──ssh -R (encrypted)──▶ mini:8765 (hub)
```

Why a public box at all: your mini sits behind home NAT — nothing on the
internet can reach it unless *some* machine with a public IP forwards to it. That
box is the relay. The mini dials **out** to it (`ssh -R`), so your home router
...
### Step 1 — mini hub, local only, no relay (lowest risk)
...
```sh
cd deploy/mac
./build-pkg.sh hub --tokens "mini:$(openssl rand -hex 24)"   # FAKE, no relay
```
Install the `.pkg` on the mini **while logged into its desktop** (not over SSH — the
postinstall refuses a headless session, by design). Then check:
- menu-bar **● green**; **Open Dashboard** → `http://127.0.0.1:8765` loads;
- file a ticket in the UI, click **Run tick**, watch it advance; `/fleet` shows `mini`.

Flip to real once the plumbing works: rebuild with `--real` and reinstall.
```sh
./build-pkg.sh hub --tokens "mini:<same-token>" --real
```

### Step 2 — the mini's own worker (already installed by the hub role)
...
### Step 4 — remote workers (Pro / Air)
Build a worker `.pkg` per machine — `./build-pkg.sh worker --device <name>` is enough.
Install on each, then in that Mac's menu-bar **Settings…** set the **Hub URL**
(`https://<ip>.sslip.io`) and pair it: on the hub's `/fleet`, **Pair a new device** →
paste the name + token into the worker's Settings. `/fleet` shows all devices online.
...
```sh
cd deploy/mac
# HUB (mini): binds all interfaces (0.0.0.0), auth on, no relay
./build-pkg.sh hub --lan --tokens "mini:$(openssl rand -hex 24) pro:$(openssl rand -hex 24)"

# WORKER: --hub, --caps, and --token are ALL optional now. The simplest build is just a
# device name — one generic worker pkg works for any machine:
...
# token from the hub's /fleet → "Pair a new device", or pre-seed with --token <pro-token>.
```
...
`settings.json` first (`config.local_setting("hub_url")`), then `INBOX_HUB`. That's why an
...
- **Do I pick a LAN IP?** No. The hub binds `0.0.0.0` (every interface), so there's no
  IP to choose. Workers address it by **name** (`<mini>.local`), not number.
- **What if the IP changes?** Nothing breaks. Bonjour re-resolves `<mini>.local` to the
...
- **Two orchestrators on the same LAN?** Fine — each has its own `.local` name and is a
  ...
  colliding names (`mini.local`, `mini-2.local`); port 8765 per-host never clashes.
- **View the dashboard:** `http://<mini>.local:8765` from any device on the LAN.
...
2. On the **mini**, make an SSH key if you don't have one:
...
3. SSH into the box and run the setup, passing a dashboard username and that pubkey:
   ```sh
   sudo bash vps-setup.sh myname "ssh-ed25519 AAAA...the-mini-pubkey"
   ```
...
# HUB pkg (the mini). Identity + relay are all optional — pair the hub itself and set the
...
./build-pkg.sh hub --device mini \
  ...
  --tokens "mini:$(openssl rand -hex 24) pro:$(openssl rand -hex 24) air:$(openssl rand -hex 24)"
...
./build-pkg.sh worker --device pro --hub https://<ip>.sslip.io --token <the-pro-token>
```

Pairing after install (hub `/fleet` → **Pair a new device** → paste name + token into that
...
agents for your user. The mini also starts the hub + the outbound tunnel.
...
- **Settings…** (worker) — edit the **Hub URL** (saved to `settings.json`), and the
  **Device** name + **Token** to pair/re-pair this machine with the hub (mint the token on
  the hub's `/fleet` → **Pair a new device**). Capabilities are hub-owned, shown read-only
...
- **Settings…** (hub) — two sections: the **relay** (**VPS host** / **tunnel user** /
  **SSH key path**, saved to `settings.json`; the tunnel wrapper reads it at launch —
  empty host = tunnel idle), and **this machine's worker** (**Device** + **Token**), so
...
URL, relay) and the worker plist (device + token), and toggles the `KILL` file. Building
...
[config.py](../../inbox/config.py) (`ROLE_MODEL_DEFAULTS`):
...
model id). Precedence: `INBOX_MODEL_<ROLE>` > `INBOX_MODELS` > `INBOX_MODEL` > default.

```sh
# bake into a machine's .pkg — the hub runs triage/scorer, workers run author/reviewer
./build-pkg.sh worker --device pro --caps '["dev"]' --hub https://<host> --token <t> \
    --models "author=opus reviewer=opus"
```

Or set `INBOX_MODEL_AUTHOR=opus` (etc.) in any runner's launchd env / shell.

## Notes / what's deliberately minimal

- **Any number of machines: 1 hub + N workers** (N ≥ 0). The hub pkg also runs a loopback
  worker, so a single Mac is a complete fleet. `mini`/`pro`/`air` here are just example
  device names — use whatever you like, one `--device` per worker pkg.
- **Capabilities are editable live in the Fleet UI** (`/fleet`), not fixed at build time.
  ...
  device to change what work it claims — no rebuild.
- **Merging a repo with no CI** is flagged on the merge card and merges only on your
  explicit approval. `--allow-merge-without-ci` (bakes `INBOX_ALLOW_MERGE_WITHOUT_CI=1`)
  additionally marks no-CI as green on the card; actually-failing checks still block.
- Ships in **FAKE mode** (`INBOX_FAKE=1`). Rebuild any pkg with `--real` once a
...
- The browser UI's only guard is the one Caddy password; the worker `/api` keeps
  its per-device Bearer tokens. Both ride HTTPS, so nothing crosses the internet
  in cleartext.
...
- **Worker self-update:** to ship an update, edit code + update **only the hub**
  (`build-pkg.sh` auto-bumps the patch version in `inbox/__init__.py` on every build —
