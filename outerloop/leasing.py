...
def reclaim_fleet(conn, tick_id):
    """Hub-side reclaim of worker leases. TTL is the authority (a stage
    ...
    rows = conn.execute("SELECT ticket_id, worker FROM lease"
                        " WHERE expires_at < datetime('now')").fetchall()
    for r in rows:
        with db.immediate(conn):
            conn.execute("DELETE FROM lease WHERE ticket_id=?", (r["ticket_id"],))
            conn.execute("UPDATE ticket SET assigned_worker=NULL WHERE id=?", (r["ticket_id"],))
            db.append_audit(conn, "recovery", "lease_reclaimed",
                            f"TTL-expired lease on ticket {r['ticket_id']} ({r['worker']})",
                            ticket_id=r["ticket_id"], tick_id=tick_id)
    return [r["ticket_id"] for r in rows]


def park_stranded(conn, tick_id):
    """Park active, unleased tickets pinned to a worker that has been offline (or was
    never seen) for PIN_OFFLINE_PARK_HOURS. A pinned ticket no live worker can claim
    would otherwise sit 'active' forever with no attempts accruing and no alarm.
    Parking is reversible from the Parked page once the worker returns."""
    rows = conn.execute(
        "SELECT t.id, t.pin FROM ticket t"
        " WHERE t.status='active' AND t.pin IS NOT NULL"
        "   AND t.id NOT IN (SELECT ticket_id FROM lease)"
        "   AND NOT EXISTS (SELECT 1 FROM worker d WHERE d.name=t.pin"
        "        AND d.last_seen >= datetime('now', ?))",
        (f"-{config.PIN_OFFLINE_PARK_HOURS} hours",)).fetchall()
    for r in rows:
        with db.immediate(conn):
            conn.execute("UPDATE ticket SET status='parked',"
                         " park_reason='pinned worker offline', updated_at=datetime('now')"
                         " WHERE id=?", (r["id"],))
            db.append_audit(conn, "recovery", "parked",
                            f"pinned worker '{r['pin']}' unseen for"
