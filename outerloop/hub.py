...
# App root = the dir containing the outerloop/ package. Derived, never hardcoded.
...
class HubHandler(WebHandler):
    ...
    def _api(self, method):
        ...
        auth_worker = None
        try:
            auth_worker = auth.resolve(conn, token)
            if path.startswith("/api/pair/"):
                # An unpaired worker has no bearer token yet — the pairing seam
                # self-guards (LAN-only, TTL, throttled; see pairing.py).
                status, obj = pairing.handle_api(method, path, body, conn,
                                                 self.client_address[0])
            elif path == "/api/intake":
                # Intake self-authenticates against settings.intake_token (webhook senders
                # have no worker token). Accept it as Bearer or ?token= (header-less senders).
            ...
            elif auth.required(conn) and auth_worker is None:
                status, obj = 401, {"error": "authentication required"}
            else:
                status, obj = api.handle(method, path, body, conn, auth_worker)
        except Exception as e:  # an API bug must not take the hub down
            status, obj = 500, {"error": str(e)}
        try:
            db.log_request(conn, auth_worker or (body or {}).get("worker"), method, path, status)
        except Exception:
    ...
    def _update(self):
        ...
        try:
            auth_worker = auth.resolve(conn, token)
            if auth.required(conn) and auth_worker is None:
                try:
            ...
            try:
                db.log_request(conn, auth_worker, "GET", "/api/update", 200)
            except Exception:
                pass
        finally:
            conn.close()
...
def _harden_exposed_hub():
    """A LAN/exposed hub locks both doors by default: worker-API auth on, and a dashboard
    password so the board isn't open to the network. Both are no-ops if already set; when no
    password exists one is generated and printed once, so exposure needs no interactive setup."""
    import secrets
    conn = db.connect()
    try:
        if db.get_setting(conn, "require_auth", "off") != "on":
            db.set_setting(conn, "require_auth", "on")
        if not db.get_setting(conn, "ui_token", ""):
            pw = secrets.token_urlsafe(12)
            db.set_setting(conn, "ui_token", pw)
            print(f"[outerloop] no dashboard password set — generated one: {pw}", flush=True)
            print("[outerloop] view it anytime: outerloop status · change it: "
                  "outerloop config ui_token <secret>", flush=True)
    finally:
        conn.close()


def _advertise(port):
    """Bonjour: let unpaired Macs on the LAN discover this hub (_outerloop._tcp).
    dns-sd ships with macOS, avahi-publish covers Linux; anywhere else discovery
    is simply off and the manual URL flow remains."""
    import atexit
    import shutil
    import socket
    import subprocess
    ...
    if shutil.which("dns-sd"):
        cmd = ["dns-sd", "-R", name, "_outerloop._tcp", ".", str(port),
               f"version={__version__}"]
    elif shutil.which("avahi-publish"):
        cmd = ["avahi-publish", "-s", name, "_outerloop._tcp", str(port),
               f"version={__version__}"]
    else:
        return
    try:
        ...
        pass  # discovery is best-effort; pairing still works by manual URL


def run_hub(host=None, port=8765):
    db.init_db()
    host = host or "127.0.0.1"
    if not auth.is_safe_bind(host) and os.environ.get("OUTERLOOP_ALLOW_PUBLIC_BIND") != "1":
        raise SystemExit(
            ...
            "OUTERLOOP_ALLOW_PUBLIC_BIND=1 only to bind a routable public IP.")
    if host not in ("127.0.0.1", "::1", "localhost"):
        _harden_exposed_hub()   # exposed on the LAN => auth on + a dashboard password, by default
        _advertise(port)        # discoverable by unpaired workers (LAN pairing flow)
    stop = threading.Event()
    threading.Thread(target=_scheduler_loop, args=(stop,), daemon=True).start()
    srv = ThreadingHTTPServer((host, port), HubHandler)
    print(f"hub on http://{host}:{port}  (API /api/*, UI /)  FAKE={config.FAKE}")
    try:
        srv.serve_forever()
    finally:
        stop.set()
