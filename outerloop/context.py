...
def _fail(ctx, k):
    ...
    # Failures rank above decisions in the Inbox; away mode must buzz for them too.
    # After commit, off-thread + best-effort inside notify.send — same as _require.
    notify.send(conn, "Ticket failed",
...
class RemoteCtx:
    ...
    epoch-stamped POST to the hub. Has no DB connection — handlers never
