...
Configure: `python3 -m inbox set notify_url https://ntfy.example.com/inbox`
(or the INBOX_NOTIFY_URL env var on the hub; empty = off, the default)."""
...
def _target(conn):
    return os.environ.get("INBOX_NOTIFY_URL") or db.get_setting(conn, "notify_url", "")
...
def send(conn, title, message):
    ...
    on purpose: a short-lived `inbox tick` process must outlive the POST (a daemon
