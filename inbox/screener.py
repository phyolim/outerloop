"""The Mac mini's PRODUCER: watch market signals and FILE analysis tickets into the
...
Tickets require [market-data, analysis] so only the mini claims them; the dedup_key
keeps a repeated signal from flooding the queue.

Config via env: INBOX_HUB (hub url), INBOX_DEVICE_TOKEN (the mini's token),
INBOX_SCREENER_SEC (poll interval)."""
...
def _file(base, token, sig):
    return client.post(base, "/api/tickets", {
        ...
        "requires": ["market-data", "analysis"], "prefer": "mini",
...
def run_screener_once():
    ...
    base = os.environ.get("INBOX_HUB", "http://127.0.0.1:8765")
...
def run_screener():
    base = os.environ.get("INBOX_HUB", "http://127.0.0.1:8765")
    token = os.environ.get("INBOX_DEVICE_TOKEN")
    interval = int(os.environ.get("INBOX_SCREENER_SEC", "60"))
