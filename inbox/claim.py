"""Server-arbitrated ticket claiming + release. Runs on the coordinator only.
...
authenticated device row's capabilities, never the request body (must-fix #5).
...
def claim(conn, device_name):
    """Atomically lease the highest-priority ready ticket this device may run.
    ...
    with db.immediate(conn):
        ...
        if over_budget(conn):  # hub-wide spend ceiling (#2) — no worker shares a tick
            return None
        dev = conn.execute("SELECT * FROM device WHERE name=?", (device_name,)).fetchone()
        ...
        for t in rows:
            if t["pin"] and t["pin"] != device_name:
                continue
            if not _caps_ok(json.loads(t["requires"] or "[]"), caps):
                continue
            epoch = (t["claim_epoch"] or 0) + 1
            conn.execute("UPDATE ticket SET claim_epoch=?, assigned_device=? WHERE id=?",
                         (epoch, device_name, t["id"]))
            conn.execute(
                "INSERT INTO lease(ticket_id, owner, pid, boot_uuid, device, epoch,"
                " claim_sub_stage, claim_status, expires_at)"
                " VALUES(?,?,?,?,?,?,?,?, datetime('now', ?))",
                (t["id"], device_name, 0, device_name, device_name, epoch,
                 t["sub_stage"], t["status"], _TTL))
            db.append_audit(conn, "claim", "claimed",
                            f"{device_name} claimed ticket {t['id']} (epoch {epoch})",
                            ticket_id=t["id"], detail={"device": device_name, "epoch": epoch})
            out = {k: t[k] for k in t.keys()}
            out["claim_epoch"], out["assigned_device"] = epoch, device_name
...
def release(conn, ticket_id, epoch):
    ...
    with db.immediate(conn):
        ...
        conn.execute("UPDATE ticket SET assigned_device=NULL WHERE id=?", (ticket_id,))
