"""The coordinator's JSON API. handle() returns (status, dict); the coordinator's
...
from . import __version__, auth, claim, config, context, db, taxonomy


def handle(method, path, body, conn, auth_device=None, intake_token=None):
    body = body or {}
    if method == "POST" and path == "/api/intake":
        return _intake(conn, body, intake_token)
    if method == "POST" and path == "/api/heartbeat":
        if not _device_ok(conn, body, auth_device):
            return 403, {"error": "token does not match device"}
        return _heartbeat(conn, body)
    if method == "POST" and path == "/api/claim":
        if not _device_ok(conn, body, auth_device):
            return 403, {"error": "token does not match device"}
        return 200, (claim.claim(conn, body["device"]) or {})
    if method == "POST" and path == "/api/op":
        return _op(conn, body, auth_device)
    ...
    if method == "POST" and path.startswith("/api/device/") and path.endswith("/control"):
        return _control(conn, path[len("/api/device/"):-len("/control")], body)
...
def _device_ok(conn, body, auth_device):
    """When auth is on, a request acting as a device must carry that device's token."""
    return not auth.required(conn) or body.get("device") == auth_device


def _heartbeat(conn, body):
    name = body["device"]
    with db.immediate(conn):
        if conn.execute("SELECT 1 FROM device WHERE name=?", (name,)).fetchone():
            # Capabilities are HUB-owned once a device is registered: the fleet UI is the
            ...
            # fields refresh here; caps change only via /api/device/<name>/control set_caps.
            conn.execute("UPDATE device SET version=?, current_ticket=?,"
            ...
            conn.execute("INSERT INTO device(name, capabilities, version, last_seen)"
                         " VALUES(?,?,?, datetime('now'))",
                         (name, json.dumps(body.get("capabilities", [])), body.get("version")))
            db.append_audit(conn, "fleet", "device_registered",
                            f"{name} joined: {body.get('capabilities')}")
        d = conn.execute("SELECT status, target_ticket FROM device WHERE name=?",
                         (name,)).fetchone()
    return 200, {"status": d["status"], "target_ticket": d["target_ticket"],
                 "hub_version": __version__, "cfg": config.hub_cfg()}


def _op(conn, body, auth_device=None):
    op = body.pop("op", None)
    if op not in context._LOCAL_OPS:
        return 400, {"error": f"unknown op {op!r}"}
    # When auth is on, a fenced write must come from the device that HOLDS the lease.
    ...
    if auth.required(conn) and body.get("ticket_id") is not None and body.get("epoch") is not None:
        lease = conn.execute("SELECT device FROM lease WHERE ticket_id=?",
        ...
        if lease["device"] != auth_device:
            return 403, {"error": "op not from the lease-holding device"}
    try:
...
def _intake(conn, body, supplied_token):
    ...
    need a device bearer token; unset token = endpoint off (default-deny)."""
...
def _tasks(conn):
    ...
    each joined to its lease (which device runs it now) and its most recent agent_run
    ...
    rows = conn.execute(
        ...
        " ORDER BY (l.device IS NOT NULL) DESC, t.score IS NULL, t.score DESC,"
        "          t.updated_at DESC LIMIT 100").fetchall()
    return {"tasks": [{k: r[k] for k in r.keys()} | {"running": r["device"] is not None}
                      for r in rows]}
...
def _terminate(conn, tid):
    ...
    with db.immediate(conn):
        ...
        conn.execute("UPDATE ticket SET claim_epoch=claim_epoch+1, status='parked',"
                     " park_reason='terminated by user', assigned_device=NULL,"
...
def _control(conn, name, body):
    ...
    with db.immediate(conn):
        if action in mapping:
            conn.execute("UPDATE device SET status=? WHERE name=?", (mapping[action], name))
        elif action == "target":
            conn.execute("UPDATE device SET target_ticket=? WHERE name=?",
                         (body.get("ticket_id"), name))
        elif action == "set_caps":
            # The fleet UI (or a client) assigns what work this device claims. Stored as a
            # JSON array; claim.py reads it as the authoritative capability set.
            caps = body.get("capabilities") or []
            conn.execute("UPDATE device SET capabilities=? WHERE name=?",
                         (json.dumps(caps), name))
            db.append_audit(conn, "human", "device_control", f"{name}: set_caps {caps}")
            return 200, {"ok": True, "capabilities": caps}
        else:
            return 400, {"error": f"unknown action {action!r}"}
        db.append_audit(conn, "human", "device_control", f"{name}: {action}")
    return 200, {"ok": True}
