...
def new_branch(ticket_id):
    return f"inbox/ticket-{ticket_id}-{uuid.uuid4().hex[:8]}"
...
def checks_green(ctx, ticket, hs):
    ...
    # that have no Actions at all (INBOX_ALLOW_MERGE_WITHOUT_CI). This relaxes ONLY the
    ...
    no_ci_msg = NO_CI_STATUS + (
        " (merge allowed by INBOX_ALLOW_MERGE_WITHOUT_CI)" if allow_no_ci else "")
