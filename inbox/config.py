...
# INBOX_HOME overrides it.
REPO_ROOT = Path(__file__).resolve().parent.parent
HOME = Path(os.environ.get("INBOX_HOME", REPO_ROOT / "data")).resolve()
...
# FAKE unless explicitly disabled. Default-safe: a fresh `tick` does nothing
# dangerous until you set INBOX_FAKE=0 and point a ticket at a real repo.
FAKE = os.environ.get("INBOX_FAKE", "1") != "0"

# checks_green() default-DENIES a merge when a repo has NO CI configured ("no CI" is
# not "green" — must-fix #3). Set INBOX_ALLOW_MERGE_WITHOUT_CI=1 to permit merging such
...
ALLOW_MERGE_WITHOUT_CI = os.environ.get("INBOX_ALLOW_MERGE_WITHOUT_CI") == "1"
...
DEVICE_OFFLINE_SEC = 120    # fleet view marks a device offline after this heartbeat gap
SCHED_INTERVAL_SEC = 3      # coordinator scheduler cadence (DB-only top-half)
...
PIN_OFFLINE_PARK_HOURS = 24 # park an active ticket pinned to a device unseen this long

# Absolute binary paths (cron's env is minimal). Fall back to PATH lookup.
CLAUDE_BIN = os.environ.get("INBOX_CLAUDE_BIN") or shutil.which("claude") or "claude"
GH_BIN = os.environ.get("INBOX_GH_BIN") or shutil.which("gh") or "gh"
GIT_BIN = os.environ.get("INBOX_GIT_BIN") or shutil.which("git") or "git"
...
def _models_map():
    """Parse INBOX_MODELS='author=opus reviewer=opus triage=haiku' into a dict. This is
    the single env var a runner (worker/hub) bakes to override several roles at once."""
    out = {}
    for pair in (os.environ.get("INBOX_MODELS") or "").split():
        if "=" in pair:
            r, m = pair.split("=", 1)
...
def resolve_model(role):
    ...
    Local env applies on the hub itself or before the first heartbeat:
       INBOX_MODEL_<ROLE>  >  INBOX_MODELS[role]  >  INBOX_MODEL (all roles)  >  role default.
    A value may be a tier alias ('haiku'/'sonnet'/'opus') or a full model id."""
    override = (HUB_MODELS.get(role.lower())
                or os.environ.get(f"INBOX_MODEL_{role.upper()}")
                or _models_map().get(role.lower())
                or os.environ.get("INBOX_MODEL"))
...
# Fleet-behavior knobs are HUB-owned (like device capabilities): a worker inherits
# them from every heartbeat, so one Mac's stray INBOX_FAKE=1 can't run a fake
