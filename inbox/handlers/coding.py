...
-> merging -> merged -> deploy_gate -> deployed -> done.
...
hard-capped; merge and deploy are always human-gated."""
...
def _kind_hint(ticket):
    ...
    test first). See inbox/taxonomy.py KIND_META."""
...
class CodingHandler(base.Handler):
    ...
    def _stage_groomed(self, ctx, ticket, hs):
        ...
        if q:
            ...
            gate.require(ctx, ticket, "clarification", q,
                         ...
                         resume_stage="groomed", pin=ticket["assigned_device"])
    ...
    # merged -> deploy_gate: always manual in v0 (no deploy executor is wired).
    def _stage_merged(self, ctx, ticket, hs):
        gate.require(ctx, ticket, "deploy",
                     "Deploy? (v0 has no deploy executor — approving just closes the ticket.)",
                     {"pr_url": hs.get("pr_url")}, resume_stage="deployed")
        return "deploy gated"
    ...
    def on_reject(self, ctx, ticket, decision):
        ...
        if kind == "clarification":
            ...
            base.set_stage(ctx, ticket, "fixing", hs, "merge_rejected",
                           "human rejected merge; back to fixing for another pass")
        else:
